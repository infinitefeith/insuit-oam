...
class Window(QMainWindow, QWidget):
    ...
    def file_open(self):
        ...
        try:
            ...
            self.textbox_input.setPlainText(name[0])
    ...
    def translate_gui(self):
        ...
        try:
            ...
            self.textbox_output.setPlainText('Translating, please waiting...')
            #刷新窗口
            QApplication.processEvents()
            print(self.my_text)
            #假设是文件路径则转换
            if isFilePath(self.my_text):
                rfcTranslate(self.my_text, self.my_text.replace('.txt', '_t.txt'))
                self.my_output = self.my_text.replace('.txt', '_t.txt')
            else:
                self.my_output = google.translate(self.my_text, dest='zh-CN')
